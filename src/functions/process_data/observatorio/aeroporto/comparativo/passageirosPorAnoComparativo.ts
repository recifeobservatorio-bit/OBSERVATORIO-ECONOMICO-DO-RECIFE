...
export const processPassageirosPorAnoComparativo = (
  ...
  data.forEach((item: AnacGeralHeaders) => {
    const passageiros = parseFloat((item["PASSAGEIRO"] || "0").toString());
    ...
    if (aeroportos.includes(aeroportoNome)) {
      ...
      if (index !== undefined) {
        processedData[index][aeroportoNome] =
          (processedData[index][aeroportoNome] as number) + passageiros;
      }
    }
  });
