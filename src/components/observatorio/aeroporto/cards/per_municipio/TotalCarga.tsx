...
const CargaTotalBox = ({ data, year, municipio, backgroundColor }: { data: any[], year: string, municipio: string, backgroundColor: string }) => {
  const getTotalCarga = (filterMunicipio: string) => {
  ...
  const totalMunicipio = getTotalCarga(municipio);
  const totalRecife = getTotalCarga("RECIFE");
  ...
  return (
    <div className="rounded-2xl p-4 flex-1 min-w-[130px]" style={{ backgroundColor }}>
      ...
      <h2 className="capitalize text-sm font-medium text-white">Carga Total (kg)</h2>
