...
const ViagensPorRegiao = ({ data, nameKey, colors, title, year }: any) => {
  ...
  const totalViagens = Object.values(processedData).reduce(
    (sum: number, item: any) => sum + item.viagens,
    0
  );

  const totalMedia = Object.values(processedData).reduce(
    (sum: number, item: any) => sum + item.viagens / item.count,
    0
  );
  ...
  const legendData = Object.values(processedData)
    .map((item: any) => ({
      ...
      value: item.viagens,
      color:
        colors[Object.keys(processedData).indexOf(item.regiao) % colors.length],
    }))
