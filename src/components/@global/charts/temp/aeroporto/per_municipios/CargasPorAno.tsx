...
interface GraficoCargaPorMesAnoProps {
  data: any[];
  municipio: string;
  year: string;
  color: Array<string>;
...
const GraficoCargaPorMesAno: React.FC<GraficoCargaPorMesAnoProps> = ({
  data,
  municipio,
  year,
  color,
  ...
  const getProcessedData = (municipioFilter: string) => {
    return data.reduce((acc: any, item: any) => {
      ...
      const carga = parseFloat(item["CARGA"]) || 0;

      if (itemMunicipio === municipioFilter && itemAno === year) {
        acc[itemMes] = (acc[itemMes] || 0) + carga;
      }
      return acc;
    }, {});
  };
  ...
  return (
    <div className="w-full bg-white p-6 rounded-lg shadow-lg">
      <ChartGrabber>
        ...
        >
          Carga Movimentada em {municipio} ({year}){" "}
...
export default GraficoCargaPorMesAno;
