...
const EmpresasZonaAtivasInativas = ({
  data,
  colors = ColorPalette.default,
  title = "Empresas por Zona",
  year,
}: any) => {
  const uniqueArrays = Array.from(new Set([...Object.keys(data['ativas']['zona']), ...Object.keys(data['inativas']['zona'])]))

  const chartData = uniqueArrays.map((key: string) => {
    const ativaNum = data['ativas']['zona'][key] || 0
    const inativaNum = data['inativas']['zona'][key] || 0
...
export default EmpresasZonaAtivasInativas;
