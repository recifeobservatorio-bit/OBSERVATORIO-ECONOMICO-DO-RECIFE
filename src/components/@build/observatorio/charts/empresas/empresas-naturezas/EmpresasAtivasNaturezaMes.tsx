...
import { processEmpresasNaturezasPorAno } from "@/functions/process_data/observatorio/empresas/empresas-naturezas/empresasNaturezasPorAno";
...
const EmpresasAtivasNaturezaMes = ({
  ...
  title = "Quantidade de Empresas Ativas no Recife",
    ...
    useEffect(() => {
      let cancelled = false;
      new EmpresasData("").fetchAllYearsNaturezas().then((rows) => {
        if (cancelled) return;
        setAnoData(processEmpresasNaturezasPorAno(rows));
      });
      return () => {
        cancelled = true;
      };
