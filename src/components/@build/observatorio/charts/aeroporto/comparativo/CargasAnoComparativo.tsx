...
import { processCargasPorAnoComparativo } from "@/functions/process_data/observatorio/aeroporto/comparativo/cargasPorAnoComparativo";
...
const CargasAnoComparativo = ({
  ...
  useEffect(() => {
    ...
    new AeroportoData("").fetchAllYearsAnac().then((rows) => {
      ...
      setAnoData(processCargasPorAnoComparativo(filtered, toCompare ?? []));
    });
    return () => {
      cancelled = true;
    };
  ...
  const mesChartData = updatedMonthChartData(processCargasAnoComparativo(data, toCompare ?? []), months ?? 1);
